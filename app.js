...
(function () {
    ...
    form.addEventListener('submit', function (e) {
        ...
        imgRequest.setRequestHeader('Authorization','client-ID  ' );
        imgRequest.send();
        ...
        articleRequest.open('GET','http://api.nytimes.com/svc/search/v2/articlesearch.json?q=${searchField}');
        articleRequest.send();
    });
    ...
    function addImage(){
      ...
      if(data && data.result && data.results[0]) {
      ...
      htmlContent='<div class="error-no-image">No images availale</div>';
    }
    ...
    responseContainer.insertAdiacentHTML('beforeend',htmlContent);
    }

    function requestError(e, part){
      console.log(e);
      responseContainer.insertAdiacentHTML('beforeend', '<p class="network-warning error-requestError">');
